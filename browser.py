...
SCHEME HANDLERS
"""
def handle_http(url, scheme, host, path, only_view_source = False):
  r = None

  # Try retrieving from cache storage
  headers, body = cache.get(url)
  ...
  if not (headers and body):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_TCP) as s:
      ...
      # handle "Transfer-Encoding" scheme to read out in chunks
      ...
      # handle "Content-Encoding" when present:
      if "gzip" == headers.get("content-encoding"):
        body = gzip.decompress(body)

      body = body.decode()

      # handle "Cache-Control" when present:
      if "cache-control" in headers:
        value = headers.get("cache-control")
        max_age = value.split("=")[-1]
        if value == "no-store":
          cache.remove(url)
        elif max_age: # ex: max-age=30 (30 seconds)
          ttl_secs = int(max_age.strip())
          cache.set(url, body, headers, ttl_secs)
...
def request(url):
  ...
  if scheme.startswith("http"):
    handle_http(url, scheme, host, path)
